...
def update_contour(image, save = False):
    ...
    # image = cv.imread('test_img_15.png')
    ...
    if image is None:
        ...
        if len(max_contour) > 0:
            ...
            cv.line(image, target_point[::-1], (image_shape[1] // 2, image_shape[0]), 255, 10)
            x, y, cw, ch = cv.boundingRect(max_contour)
            cv.rectangle(image, (x, y), (x+cw, y+ch), (0, 0, 255), 2)      
        ...
        # cv.imwrite('test_img_15_FIXED.png', image)
