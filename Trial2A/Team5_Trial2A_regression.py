...
def update_contour(save = False):
    ...
    for i in range(1, 11):
        ...
        if image is None:
            ...
            # Change to hsv and take out glare
            hsv = cv.cvtColor(image, cv.COLOR_BGR2HSV)
            glare_mask = cv.inRange(hsv, (0, 0, 240), (179, 40, 255))  
            glare_img = cv.bitwise_and(image, image, mask=cv.bitwise_not(glare_mask))
            cv.imwrite('img_glare.png', glare_img)
            
            img_float = glare_img.astype(np.float32)
            kernel_size = 101 # must be odd
            local_average = cv.blur(img_float, (kernel_size, kernel_size))
            local_average = np.where(local_average == 0, 1.0, local_average)
            normalized = (img_float / local_average) * 128.0
            normalized = np.clip(normalized, 0, 255).astype(np.uint8)
            cv.imwrite('img_normal.png', glare_img)
            ...
            contours = rc_utils.find_contours(normalized, BLUE[0], BLUE[1])
