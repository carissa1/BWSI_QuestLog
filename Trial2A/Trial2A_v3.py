...
def update_contour(save = 'False'):
    ...
    if image_lower is None:
        ...
        if len(max_contour_lower) > 0:
            ...
            contour_center_lower = (contour_center_lower[0], contour_center_lower[1] + OFFSET) # + CROP_MID[0][0]
            ...
            rc_utils.draw_contour(image_lower, max_contour_lower, color=(255, 0, 0))
            rc_utils.draw_circle(image_lower, contour_center_lower, color=(150, 0, 150))
          

        contour_center_upper = None
        if len(max_contour_upper) > 0:
            contour_center_upper = rc_utils.get_contour_center(max_contour_upper)
            contour_center_upper = (contour_center_upper[0], contour_center_upper[1] + OFFSET) #  + CROP_FLOOR[0][0]
            ...
            rc_utils.draw_contour(image_upper, max_contour_upper, color=(0, 255, 0))
            rc_utils.draw_circle(image_upper, contour_center_upper, color=(150, 150, 0))
...
def update():
    ...
    global kd
    global last_angle
    global angle_near, angle_far
    global error_near, error_far
    ...
    angle_far = 0
    error_near = 0
    if contour_center_lower is not None:
        setpoint = rc.camera.get_width() // 2
        present_value = contour_center_lower[1]
        error_near = setpoint - present_value
        angle_near = kp * error_near + kd * (error_near - last_error) / rc.get_delta_time()
        last_error = error_near
    else:
        angle_near = last_angle

    if contour_center_upper is not None:
        setpoint = rc.camera.get_width() // 2
        present_value = contour_center_upper[1]
        error_far = setpoint - present_value
        angle_far  = kp_far * error_far
    else:
        angle_far = 0
    
    if contour_center_lower == None and contour_center_upper == None:
        angle = last_angle
    else:
        angle = rc_utils.clamp(angle_near*0.7 + angle_far*0.3, -1, 1)
...
def update_slow():
    ...
    print("INDEX: ", indx)
    print("     SPEED: ", speed, "ANGLE: ", angle, "ERROR: ", error_near, " far: ", error_far)
    print("     angle_near: ", angle_near, "angle_far: ", angle_far)
