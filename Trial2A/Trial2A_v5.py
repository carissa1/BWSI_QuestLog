...
def update():
    ...
    if contour_center_lower is not None:
        near_error = setpoint - contour_center_lower[1]

        if contour_center_upper is not None:
            far_error = setpoint - contour_center_upper[1]
            raw_error = NEAR_WEIGHT * near_error + FAR_WEIGHT * far_error
        else:
            raw_error = near_error  # if can't see far line, just use near for error
